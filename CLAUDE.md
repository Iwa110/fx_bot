...
- **★撤退基準の再設計(2026-06-30追補, ユーザー指摘「無レバなら塩漬け可」)= 結論が反転**: `intervention_playbook_bt.py` に撤退ポリシー比較(maxDD深さ/円・BE/元水準復帰日数)を実装。**abort が一時含み損を"損失確定"させる張本人**だった。KNOWN介入 n=9 で **abort(-3円)=-185k / hold_ph(塩漬け→元水準)=+1,257k・9/9全回復 / hold_be(建値決済)=+4k**。全発火でも hold_ph +2,371k(キャリー込)。**無レバ塩漬け(hold_ph)が正解**。**但しコスト**=最大含み損 2022-10:**21.2円/240営業日**・2024-07:**18.7円・データ内未回復(OPEN, ~695日=構造転換テール)**、クリーンな2022-09ですら14.2円沈む。**回復DDが最大21円=21円未満の"賢い中間abort"は存在せず二択**(無abort塩漬け or 勝ちを切る)。テール防御は価格でなく**ファンダ回路遮断**(BOJ持続利上げ/米景気後退で金利差消滅=マクロ前提崩壊時のみ手動撤退)。monitorに `--exit-policy {hold_ph(既定)/abort/hold_be}` 実装。**実lotは maxDD~21円を無レバで耐える資本から逆算**(demo 0.30lotで~640k JPY評価損)。
- **細かい足(tick/1m)**: 案Dの塩漬け/撤退判断は週〜年スケールで **5m(むしろ日足)で十分**=本結論に影響なし。tick/1mが効くのは案B(速い反応型ショート=既に弱い)の入口精度のみ。当環境はegressポリシーが `freeserv.dukascopy.com` を403拒否で取得不可 → ローカル `.venv_dukas` で介入日窓のみ取得可: `python optimizer/fetch_dukascopy_ohlc.py --tf 1m --pairs USDJPY ...`(要 `INTERVAL_MIN_1`/`INTERVAL_TICK` 対応・日付範囲指定)。優先度低。
- **★スワップ(金利差)加味の long塩漬け損益(2026-06-30再追補, ユーザー依頼)**: `intervention_playbook_bt.py` のキャリーを一律2.5%→**米日政策金利差の時変ステップ関数で日次積算**(USD Fed上限/JPY BoJ, build_policy_rates.py同値+2026補完, haircut0.85)に改修。**KNOWN介入 HOLD_PH: price +634,950 + swap +927,651 = total +1,562,601円(demo0.30lot, n=9, swapが約59%)**。**塩漬けが長い介入ほどswap寄与が支配的**=2022-10(337日保有:price+38.7k/swap+174.6k)・2024-07-11(695日OPEN・price未回復+51.7kだがswap+301.5k=total+353k)・2024-07-12(swap+247.9k)。**無レバ塩漬けの最大含み損(〜21円/〜64万円@0.30lot)を、保有中のswapが相当程度オフセット**=ユーザー仮説(塩漬けで金利を取る)がデータで支持。⚠️**重要caveat=キャリーはレジーム依存**: +927kの多くは2022-24の高金利差(4-5%)の産物。**現状は差2.75%かつBOJ利上げ/Fed利下げで縮小方向**=今後の介入の塩漬けは2022-24比で半分程度のswapしか付かない。極端にBOJ利上げ×Fed利下げが進めば差が消え/逆転しlong swapが負転する尾リスクも(塩漬け前提が崩れる)。成果物: `intervention_playbook_bt.py`(RATE_USD/RATE_JPY・carry_jpy・_result.csvにcarry_yen列)。
- **★実口座サイジング・利益試算(2026-06-30, ユーザー「live実施・現行2.75%でサイジング・BOJ連続利上げ撤退」)= `optimizer/intervention_sizing.py`**: price損益はレジーム非依存(demo実測を1.0lot線形化), carryを現行2.75%(と縮小1.5%)で再計算。**決定的な現実=単一ポジ運用(monitorは塩漬け中に新規検出せず)だと20発火中8しか取れない**(長期塩漬けが後続を占有)。**1.0 total lot 年率: SEQ(現実)/2.75%=約318k JPY**(price202k+carry117k) / SEQ/縮小1.5%=約265k / ALL(上限)/2.75%=773k。サイジングは無レバ塩漬けの束縛=耐えるDD(実測max21.2円→ストレス30円/lot=300万JPY)で逆算: 例 自己資本300万・許容DD50%→safe 0.50lot→年間**約16万JPY**(SEQ/2.75%, worst含み損150万)。**月利30万(年360万)には total~11.3lot/自己資本~6,800万JPYが必要**(介入が不定期+単一ポジ捕捉が低いため資本効率は低い)。⑤単発頻度不規則=年率は平準化した見かけ。
- **BOJ連続利上げ撤退トリガ実装(monitor)**: 米日政策金利差テーブル(手動追記)+ `--live-total-lot` でlive lot指定。**金利差<=RATE_EXIT_TH(既定1.0%)で塩漬け強制手仕舞い**(carry消滅/逆転の回路遮断)。歴史的に介入期は差>=2.75%で一度もbindせず=過去試算不変・将来テールの保険。⚠️諸刃=発火時にprice未回復なら含み損を実現。live は `--live-total-lot`(sizingのsafe_lot)必須・未指定は拒否。**正直な総括=実体は汎用キャリー・エッジで介入固有でない/現行carryは縮小方向/資本効率低い/n極小。demo forward先行を強く推奨**。成果物: `intervention_sizing.py`。
