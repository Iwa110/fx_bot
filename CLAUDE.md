...
Grid戦略の改善を生成AI(Claude Code)で自律的に回す「ループエンジニアリング」基盤を設計・実装(`optimizer/grid_loop_engineering_design.md`セクション7確定事項)。過学習を機械的ゲートで防ぐことを最重要制約とし、コアBT`grid_floatstop_bt.py`はSHA-256凍結・read-only保護。
- **Phase0(基盤)**: `optimizer/loop/`に台帳(`ledger.py`, JSONL追記型)・6ゲート判定(`gates.py`: gate1 IS/OOS PF符号+decay、gate2標本数/年、gate3 plateau変動率(崖スパイク排除)、gate4月次OOSバジェット、gate5墓場照合、gate6 WFO年次最小PF)・MC必要資本(`mc_capital.py`, 月次ブロックブートストラップ)・評価CLI(`evaluate_candidate.py` explore/confirm/card)・カード生成(`card.py`)・ハッシュガード(`hash_guard.py`)を実装。コアBTに`tp_mult`/`tp_level_mults`(ラダー深さ別非対称TP)/`ptp_frac`+`ptp_mult`(部分利確)を追加(全キーデフォルトOFF、静的一致テスト`test_grid_floatstop_static.py`で凍結基準との完全一致を検証)。
- **Phase1(初回仮説ラン)**: gain側ファミリー`grid_ladder_depth_asymmetric_tp`(`tp_level_mults`, steepness 0〜0.90の7点sweep=浅legは絞り深legは緩めるプロファイル)をAUDCAD/CADCHF/EURGBP/AUDNZDでexplore→confirm。構造的理由=ラダーのk番目のレッグはアンカーからk段乖離した時点で建つため深いレッグほど中心からの乖離が構造的に大きく、捕捉しうる回帰距離も平均的に大きいはず(AUDCAD 3段階Z-score MRエンジンの`[[project_mr_exit_depth_scaleout_20260716]]`知見と方向は一致、別エンジンでの独立再検証)。**4ペア全てgate6(WFO年次最小PF≥1.0)で不合格、gate_passedゼロ**:
  - AUDCAD: IS1.39/OOS1.54(decay-0.10=OOSの方が良い)・plateau1.9%・gate1-5全PASS。唯一n=39の2026部分年foldのみPF0.91で不合格(フル4年は全て>1.0=最も惜しい結果)。
  - CADCHF: IS1.49/OOS1.40。2024**通年**foldがPF0.91で不合格(部分年でない実質的な脆弱性)。
  - EURGBP: IS1.46/OOS1.24。2022**通年**foldがPF0.76で不合格。
  - AUDNZD: coreBT表現(v8のregime_short/mom/cull/taper抜きのbaseline)自体がIS<1.0の既知の限界ペア。gate1(OOS PF≥1.2)にも抵触、WFO最悪0.47。
  - 4ペア共通でreq_cap_99・p_loss_5yrはbaseline比±5%以内(致命的なリスク悪化はなし)。律速は一貫してgate6のみ。
- **運用中に発覚したgate実装ギャップを2点修正**(設計意図はそのまま、実装のみ訂正): ①gate4(月次OOSバジェット)をhypothesis単位カウント→**family_tag単位**に変更(同一ファミリーを複数ペアで検証しても1件、ユーザー指示)。②gate5(墓場照合)の台帳由来closed-family判定を**(family_tag, pair)にスコープ**変更(1ペアの確定closeが他ペアのexploreを即ブロックしていた不具合を修正、静的`graveyard.json`の無条件ブロックは維持)。
- 本番`optimizer/loop/ledger.jsonl`稼働開始(28仮説・32レコード)。今月のOOSバジェットは4件中**1件消費**(残り3件)。`grid_ladder_depth_asymmetric_tp`の`graveyard.json`への追加(恒久クローズ登録)は見送り=今回は単一のsteepness-sweep構成での全滅であり、異なる中心/範囲や決済併用での再検証余地を残すため。台帳の`(family_tag, pair)`ブロックにより同一構成の再テストは自動的に弾かれる。
- **次アクション(任意)**: ①AUDCADの惜敗(部分年foldのみ不合格)を踏まえ、WFOの部分年fold扱い(除外/加重/最低日数見直し)の設計是非をChatで再検討。②残バジェット3件で`ptp_frac`+`ptp_mult`(部分利確)ファミリーを次に試す。③あるいは`tp_level_mults`で異なるsteepness中心/範囲を再探索。

### ★F 構造健全性モニタ 設計 → 脚相関ベースの自動停止は Stage0 却下(E1を2次モーメント軸で再現)・監視は「実現パフォーマンス反応の決定論モニタ」に集約（2026-07-16）
確定エッジ相関クロスMR(Grid4本+AUDCAD tiered)の唯一の存亡リスク=**2脚の共有ドライバ関係崩壊→クロストレンド化→MRが轢かれる**への監視層を設計。**位置づけを既存Close結論と非衝突に固定**: E1(`[[project_grid_regime_persistence_e1_20260616]]`)が殺したのは「trailing地合いでforward PnL予測(→サイジング/エントリー=エッジ)」。Fは別物=「共有ドライバの生死(2脚リターン相関=2次モーメント, E1未検証軸)を**予測でなく検知**し、劣化が持続すれば停止&前提再検証のops判断に使う」。「クロスvariance-ratioと退化し無力」の可能性ゆえ Stage0 で先に検定。
- **健全性H = -rollcorr(2脚 D1 log-return)**(高=2脚相殺=健全レンジ)。AUDCAD=AUDUSD×USDCAD / CADCHF=USDCHF÷USDCAD / AUDNZD=AUDUSD÷NZDUSD / EURGBP=EURUSD÷GBPUSD(全脚 D1_dukas 11.5年)。`optimizer/mr_health_monitor_diag.py`。
- **Stage0=却下**: ①A予測(H[m-1]→MR純益)全ペア不安定・符号反転=E1再現で予測不可。②A機構(H→クロス|変位|)はEURGBP(最弱ペア)のみ安定=機構弱い。③B同時点もほぼ不安定=coincidentも不可。④**D最悪月profile: MR最悪月の平均H≈平常月**(AUDCAD OOS worst+0.77/rest+0.73)=検知すべき劣化を捉えられない**決定打**。⑤C持続性のみ良好(H autocorr0.72-0.95・低H連続2-16ヶ月)だが動かす信号無し。**機構=確定エンジンがCIゲート/z構造でトレンド窓を自動回避→実現MR損益が生トレンド・レジームから乖離し脚相関で捕捉不能**。守りたい突発ブレイクは`[[project_carry_crash_hedge_20260615]]`「突発ジャンプは事前検出不能」系。
- **採用アーキテクチャ=実現ベース決定論モニタ**(`optimizer/mr_health_monitor_design.md`, killswitch`[[project_grid_exit_adoption_killswitch_20260618]]`の運用化): per-pair①ローリング12ヶ月実現PF②現DD対req_cap(lot1: AUDCAD691k/CADCHF2.27M/AUDNZD1.25M/EURGBP2.28M)③FSスリッページ④TP/エントリー数。**KILL=12moPF<1.0 or 現DD>req_cap_99 or FSスリッページ>設定1.3倍**。DE-RISK=PF1.0-1.2灰色帯かつDD>req_cap_50。PROMOTE=3ヶ月∧TP≥30∧FS発火∧PF>1.2。バスケットは合算DD対2.80M+「4本の深DD窓非重複(相関≈0)」実現検証。cadence=日次(sync_history.py/07:30 task相乗り)・新規常駐なし・閾値BT由来凍結(緩め禁止)。**脚相関Hはダッシュボード文脈値のみ(PnLと無相関ゆえ自動トリガ禁止)**。詳細メモリ`[[project_mr_health_monitor_20260716]]`。

### ★gain側survivor(B1 深さ条件付きTP / B2 段階利確)検証 → 全ペア一律の底上げは無し・AUDCAD deep-run TPのみclean Pareto改善・「決済スタイル∝回帰速度」を2軸で確認（2026-07-16）
E1(`[[project_grid_regime_persistence_e1_20260616]]`)が「残る上積み余地=gain側B1/B2とエントリーD2」とした gain側2軸を、AUDCAD確定MRエンジン `dynamic_lot_mr_bt.run_bt_tiered3` を**一切改変せず**per-tier/scale-out版を別ファイルに書いて検証(退化構成で原エンジンと**機械精度一致assert**: n=466 net=1647.244053 完全一致)。確定構成固定(n40/z_tiers2.0/2.5/3.0/lot0.2/0.3/0.5/z_stop4.5/max_hold48/vol_throttle0.70x0.5)・決済のみ可変・全4本へ同一適用で構造性(転移)点検・IS2015-21/OOS2022-26/年次WFO。
- **B1 深さ条件付きTP(`optimizer/mr_depth_tp_bt.py`)**: tp_zs[t]=各ティア利確Z(short exit=MA+tp_z*SD; 正=早利確/負=オーバーシュート)。**AUDCAD=`[0.3,0,-0.3]`(浅leg早利確・深legをMA先まで走らせる)で clean Pareto改善**: IS1.24→1.29 / OOS2.57→3.05 / full1.61→1.75 / DD-15% / net/DD11.3→14.7 / wfoMin1.22→1.18(全fold>1.0維持)・プラトー(0.25〜0.5で滑らか=崖でない)・IS-selectable。逆方向`[-.5,0,.5]`はwfoMin0.96へ悪化(方向確認)。**CADCHFは同tp_zsで真逆にnet負転**(deep-run有害)=単一tp_zsは転移せず構造エッジでない。AUDNZDは`[.5,0,0]`でwfoMin0.94→1.02。EURGBPはIS<1で選択不能。
- **B2 段階利確/scale-out(`optimizer/mr_scaleout_bt.py`)**: schedule(scale_zs降順・fracs原ロット比)を全レッグ一律(深さ非依存=B1と直交)。**AUDCAD=全schedule(前傾/後傾/2段/3段)でfull net -10〜-35%**・WR72→76%上昇だがnet低下・wfoMin1.0前後=「高勝率≠高PF/早期決済が回復益を殺す」signature再現→**AUDCADにscale-outは有害**(B1「深legを走らせる」の真逆で整合)。CADCHFも一律scale-out無効(baseline OOS1.03自体が限界)。**AUDNZD=B2b`[.5σ/MA 50:50]`でwfoMin0.94→1.11・net維持**=限界ペアfold頑健化(唯一の正の使い道, B1cと同等)。
- **★核心=B1(深さ軸)とB2(段階軸)が独立に同結論**: 回帰速度勾配 AUDCAD(遅・クリーン)→AUDNZD(中)→CADCHF(速)に対し最適決済が「走らせる→軽く刻む→刻む」と単調変化。横展開メモ(`[[project_mr_tiered_transfer_20260630]]`)の「CADCHF最速→構成B(部分利確)最適」と完全整合。新パラメータを増やすのでなく**既測 T_reg(回帰速度)に決済スタイルを従属させる**のが正しい一般化=curve-fitでなく構造。
- **D2 平均回帰確認エントリー(`optimizer/mr_confirm_entry_bt.py`)=棄却**: |Z|≥z_inで即フェードせず反転確認(zturn/candle/ema/rsi, 確定足・次足約定)を待つ。全ペア・全モードでWR上昇・DD/net低下だが**選択可能なAUDCADでfull PF実質不変(1.61→1.63)のままnet-28%(取り逃し)**=Phase3「確認はWR上げるがPF不変〜悪化」がtiered文脈でも再現。限界ペアAUDNZDはOOS0.82-0.98/wfoMin0.49-0.73で積極的悪化。「OOS2.70に見える」は高WRの見かけ=IS PF上がらずex-ante選択不能。**確認/フィルタはWR・露出を制御できてもエッジ生成器でない**。
- **★判定=E1残survivor(B1/B2/D2)完全決着**: "全ペア一律のgain側底上げ"は存在しない。唯一の耐久的改善=**AUDCAD B1 deep-run TP `tp_zs≈[0.3,0,-0.3]`**(forward-test構成候補, PF1.61→1.75)。**B2はAUDCADに使わない**(net-worse)。AUDNZDはB1c/B2bでwfoMin>1.0補強可(限界的)。CADCHF/EURGBPは決済側で改善なし=決済A据置。**D2(エントリー精度)全滅=gain側・エントリー精度側のBT探索は枯渇**。次アクション=①AUDCAD deep-run TPをz_stop/vol_throttle交互作用+MCで必要資本再確認→forward-test構成確定 ②vps/mr_monitor.py反映はforward-test後 ③残る探索軸はF(構造健全性モニタ=予測でなく存亡リスク検知のops, エッジ検証でない)のみ。詳細メモリ`[[project_mr_exit_depth_scaleout_20260716]]`。

### ★★「円安は続くか」構造ドライバのOOS予測力検定 → Stage0却下=キャリー・スリーブは静的マクロベットで実装不可・carry系打ち切り（2026-07-13）
スワップ連合艦隊(`[[project_carry_fleet_swap_basket_20260713]]`)の唯一のエッジ=「USDJPY 115→160 の円安」に乗った裸のJPYショート(per-pair分解でJPYファンディング+285k vs CHF-60k)だった前提そのものを検証。中核の問い=**円安は構造ドライバで予測/タイミング/サイジング可能か(=(A)米日金利差キャリー継続力 vs (B)PPP/水準ストレッチ平均回帰力の優劣を出所別に分離)、それとも取れない裸の方向ベットか**。成果物=`optimizer/jpy_regime_diag.py`(Stage0診断のみ、本BT不要)+`jpy_regime_diag_result.csv`/`_panelB.csv`/`_panelC.csv`。データ=USDJPY D1(2014-12〜2026-06, 119.6→160.0)+policy_rates.csv(USD/JPY decision-date段関数)+WTI D1。規律=IS2015-21凍結/OOS2022-26、全ドライバt既知、overlap forward returnゆえp値でなくIS/OOS符号安定性で読む(commodity_fx_lag_diag同型)。**留保=実質金利差(本命候補)はCPI無で名目政策金利差のみ、REER/PPPはtrailing750d水準ストレッチz代理(真のPPPでない)**。
- **Panel A(ドライバ(t)→forward USDJPY return Spearman, H=21/63/126/252)=carry仮説ドライバ全滅**: ①**rdiff(金利差水準)=全Hで仮説と逆符号(rho IS -0.03〜-0.17/OOS -0.09〜-0.21)**=高金利差は割高水準と同居しforward下落で方向を与えない(平均回帰力に汚染)。②**★rdiff_chg126(金利差拡大)=全HでIS↔OOS符号反転(IS -0.09〜-0.12/OOS +0.10〜+0.28)=決定的kill**。「金利差拡大→JPY安」はOOS(2022-24利上げ窓)限定で、IS(2015-21, 2016-19 Fed利上げ0.25→2.5%を内包・2016はBrexitで金利差拡大中にJPY大反発)では逆符号=ex-ante選択不能。③oil_mom63のH=252 IS 0.36は2015-16単一窓(他Hほぼ0)。④ppp_dev(平均回帰,割高→反落)=rho両窓で負(符号一致・唯一の頑健力)。
- **Panel B(Δrdiffの同時点 vs 予測)**: IS=coincident/predictive共に弱負(相関そのもの無)。OOS=coincident+0.15/+0.26・predictive+0.15/+0.20=コモディティ「lag0>>lag+1(織り込み済)」型でなく、より悪い「関係がOOS窓だけ存在しISで反転」型。
- **Panel C((B)平均回帰の左テール)**: 高ppp_dev(割高)クインタイルの mean_fwd は両窓で>0(0.003〜0.09=割高でもcarryが押し上げ続ける)、worst のみOOSで拡大(H=63 -0.146/H=126 -0.125=2024-08 carry-crash)。**割高は反落の"平均"でなく"左テール(carry-crash)"だけを予測=事前タイミング不能**(`[[project_carry_crash_hedge_20260615]]`「突発政策ジャンプは事前検出不能」と整合)。合成信号(事前登録等加重)もIS/OOS両>0.1のHゼロ。
- **★Stage0判定=却下(裸の方向ベット, reject条件3充足)**: ①rdiff_chg IS↔OOS符号反転 ②(A)金利差は2022-24窓限定でselectableでない ③(B)平均回帰項が唯一頑健だが(A)と逆向き=円安局面でショートを外す方向で純方向が定まらない。→**キャリー・スリーブ(carry_fleet v2/v3)は静的マクロベットに留まり、ドライバによるゲート/サイジングでは改善不可=実装不可**。円安が続くかは「米日金利差が開き続ける∧リスクオン∧BoJが動かない」の共起への裸のマクロ・ベットで、構造ドライバから事前に取れるタイミング/サイジング要素でない。本BT(jpy_regime_bt.py)は不要。**carry系リソースは打ち切り、確定FXエッジ(Grid確定4本 `[[project_grid_joint_stepb_20260615]]` + AUDCAD MR `[[project_audcad_mr_tiered_strategy_20260630]]`)へ集約継続**。将来cryptoと同じく、円安を再考するなら「金利差水準でなく、レジーム共起を事前判別できる建付け」以外は入口で却下。詳細メモリ`[[project_jpy_regime_persistence_20260713]]`。

### ★★crypto拡張 凍結確定: 候補A/B/C全Close。C(トレンドフォロー)を税第一級ゲートでpre-registered kill=構造ストーリー最強でも繰延buy&holdの税優位に敗北・vps実装候補ゼロ（2026-07-11 第2セッション）
crypto拡張の最終決着。C0-C3(候補A ETH/BTC比率MR / 候補B ファンディング・ベーシス)完走後、Chatで「候補C(トレンドフォロー)を税を第一級制約に一度だけ決定的に検証し、超えなければ凍結」を確定して実施。**大前提=税の定理**: 国内crypto雑所得55%・繰越/通算なし。含み益で繰延できるbuy&holdは最終清算で一度だけ課税、能動戦略は毎年realizeで勝ち年課税・負け年救済なし→能動の課税ベースΣ(勝ち年)≥buy&holdの複利済み最終純利益=能動は「同じ税引後資産」を得るだけでも余分に税引前で稼がねばならない(税ハードル)。
- **Stage0 税ゲート(`optimizer/crypto_tax_gate.py`, 本BT前に単独実行)**: after_tax_active(年次realize)/after_tax_bh(繰延最終清算のみ)を実装し「buy&hold税引後を追い抜く必要能動税引前CAGR」を数値化。**ハードルはレジーム依存**=強気IS(2017-21, buy&hold BTC10.8x/ETH12.2x)で必要能動CAGR **89-96%**=非現実的、弱気/レンジOOS(2022-26)は低い(ETH buy&holdは元本割れ0.48xで能動は横ばいで勝てる)。→その低ハードル窓こそ「1クラッシュ回避運」の温床でbar④で殺す設計に直結。成果物`crypto_tax_gate_result.csv`/`_sensitivity.csv`。
- **★候補C本体(`optimizer/crypto_trend_bt.py`)=pre-registered Close(bar①FULL/IS ∧ bar④ 不達)**: low-turnover最優先(税ドラッグ最小)でSMA200/100レジーム・Donchian50・BTC↔ETHローテをlong-only/次足始値/フルコスト/確定足shift、**全て税引後でbuy&hold(繰延)と比較**。**bar④(両レジーム)不達=決定打**: 強気ISで**全変種がbuy&hold税引後に届かず**(最良ETH:SMA100でも税引後3.5x vs BH6.0x)、勝つのは弱気OOSだけ(buy&holdが損した窓の回避=ex-ante判別不能なレジーム運)。**bar①(第一級)不達=FULL**: honest full cycle(2017-26)でbuy&hold税引後が圧倒(BTC:SMA200 税引後1.67 vs BH7.28/ETH2.32 vs3.23)。**機構(WFO税引後)**=戦略は上げ年2023/24をbuy&holdに負け・下げ年2022/26だけキャッシュ退避で勝つ(BTC fold勝ち2/5・ETH4/5だが全て下落年)。**bar③(DD)はPASS**(戦略maxDD-33% vs buy&hold-67%=本物のリスク削減)だが第一級bar①落ちで無意味。**★ゼロコストでもFULL税引後 BTC1.97 vs7.30/ETH2.62 vs3.24=コスト非依存の構造kill**(敗因は繰延buy&holdの税効率+トレンド部分捕捉)。成果物`crypto_trend_bt_result.csv`。
- **補助診断D(`optimizer/crypto_dipbuy_diag.py`)**: 大下落日(下位20%)翌日snapback+0.34%/+0.38%(t≈1.7-2.1・弱)・日次autocorr-0.05だが**往復0.6%コスト差引後は負(-0.26%/-0.22%)=取引不能**(コモディティ→FX lag診断と同型のStage0型却下)。成果物`crypto_dipbuy_diag_result.csv`。
- **★最終判定=crypto拡張 凍結**: 候補A(Close)/B(海外拡張非推奨)/C(pre-registered kill)全て墓場。構造ストーリー最強のC(繋留無し→持続トレンド)ですら、税(繰延buy&holdの構造優位)が能動トレンドの部分捕捉+年次課税を上回りfull cycleで敗北=事前予想(税引前で勝っても税引後で消える)どおり。**土日稼働は動機でありエッジでない、を数字で確定。vps実装候補ゼロ。リソースは確定FXエッジ(AUDCAD MR + Grid確定4本 + CADCHF追加demoフォワード)へ集約**。将来cryptoを再考するなら、繰延税優位を能動が構造的に埋める建付け以外は入口で却下。

### ★crypto拡張C0-C3(第1セッション): 候補A(ETH/BTC比率MR)=構造的エッジ皆無でClose / 候補B(ファンディング・ベーシス)=本物だが税+海外リスクで薄すぎ・海外拡張は非推奨（2026-07-11）
