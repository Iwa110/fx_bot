...
6. [SMA Squeeze Play](#6-sma-squeeze-play-sma_squeezepy-v46)
...
**v29 バグ修正・インシデント記録 (2026-06-05〜06-08):**
- **不具合**: `is_in_cooldown` が `DEAL_REASON_SL` のみ対象のため、T_max強制決済（`DEAL_REASON_EXPERT` / `BB_time_stop` コメント）後にクールダウンが発動せず、未反転BBシグナルへ毎分即時再エントリーを反復。
- **被害**: 2026-06-05 NFP時間帯(UTC 11-12時)にEURJPY 71件・-549,300円の異常損失発生。バグポジションは2026-06-08に+1,331,620円で決済（偶発的回収）。
- **修正**: `is_in_cooldown` に `BB_time_stop` コメント判定を追加。SL/T_max 両方で COOLDOWN(15分)を適用。v29実装・push済み。
- **EURJPY Phase1 nリセット**: バグ71件を無効化。**正常サンプル n=9 からリセット（2026-06-08）**。
...
## 6. SMA Squeeze Play (sma_squeeze.py v4.6)
...
| クールダウン | 180分/ペア（**v4.6: 強制決済(TMAX/force-close/slope-exit)時に決済時刻起点で再武装**。エントリー時刻基準のままだと長時間保有後の強制決済で失効済みとなり即再エントリーしうるギャップを是正） |
...
| Grid CADCHF | 20260038 | `GRID_CDC` | grid_monitor.py |
...
| クールダウン | **v2: 168時間(1週間)/ペア**。決済(SL/TP/max_hold)後、deal履歴ベースで同銘柄の再エントリーを禁止。週次COTシグナルが残る限り即再エントリーして負けテーゼ反復・max_hold空回りを起こす不具合を是正（1ペア1週間1エントリー上限） |
...
3. **ポジション確認**: 同一ペアにポジションなし、総ポジション < 3、**クールダウン外（直近168hに同銘柄の決済なし / v2）**
...
- **dir_mode**: `both`/`long_only`(carryグリッド=USDJPY/NZDJPY, short構造的壊滅)/`regime_short`(close>SMA(sma_period)の上昇局面のみ新規short停止=AUDCAD/AUDNZD/CADCHF)。
- **short_lot_mult**: ソフトな方向チルト（EURGBP=0.5）。**tp_mult**: per-leg TP距離=grid_width×tp_mult（EURGBP=0.8）。

知見=救済の2類型: ①JPYクロスcarry-grid(long-only)=NZDJPY/USDJPY ②相関クロスgrid(両側/レジーム+combo)=AUDCAD/AUDNZD/EURGBP/CADCHF。GBPJPY/CHFJPY/EURUSDは構造的に救済不可（No-Go据置）。
...
| CADCHF | 1.00 | -943,000 | -1,886,000 | -943,000 | demo / v8 FT(相関クロス) |
...
- **v7注**: float_stopは深いラダーのテール保険としてのみ有効。浅いラダーでは緩め固定（非発動）が最適で、きつくすると回復可能ポジを切りPF・DD悪化。

### バックテスト結果詳細（v7 / grid_floatstop_bt.py + grid_floatstop_sweep.py: float-stop込みフル2年 2024-04〜2026-04）

旧BT（grid_levels_bt.py / 旧仕様）は float-stop 未実装・lot0.02 のため過大評価だった。以下は float-stop を実装しライブlotで回した結果（net=PF, 損切り込み, v7確定構成）。

| ペア | 構成(ci/atr/lv/fs) | PF | net円 | n_tp | n_b48 | worst単発 | max_dd |
|------|--------------------|----|-------|------|-------|-----------|--------|
| GBPJPY | 61.8/1.5/7/-1.5M | 1.96 | +5,965,956 | 269 | 0 | -1.62M | 3.40M |
| CHFJPY | 65/1.0/3/-1.5M | 1.51 | +1,412,119 | 146 | 9 | -0.65M | 0.61M |
| NZDJPY | 61.8/1.5/7/**-1.0M** | **2.36** | **+4,546,370** | 307 | 2 | -1.05M | 1.05M |
| AUDCAD | 65/1.0/5/**-750k** | **4.01** | **+5,499,879** | 332 | 5 | -0.45M | 0.45M |
| NZDUSD | 61.8/2.0/7/-15k | 1.81 | +15,905 | 121 | 0 | -0.02M | 0.02M |

**バージョン別 純益の推移（反転→float_stop最適化）**

| ペア | 旧(lv7原型) | v6 | v7(float_stop最適) |
|------|-------------|-----|--------------------|
| CHFJPY | 0.70 / -3.76M | 1.51 / +1.41M | 1.51 / +1.41M（据置）|
| NZDJPY | 0.96 / -0.43M | 1.65 / +1.92M | **2.36 / +4.55M** |
| AUDCAD | 1.26 / +2.63M | 2.75 / +4.28M | **4.01 / +5.50M** |

**float_stop感応度（CHFJPY ci65/atr1.0/lv3, 緩め固定が最適の証跡）**

| float_stop | PF | net | n_fs | maxDD |
|-----------:|---:|----:|-----:|------:|
| -300k | 0.98 | -89k | 14 | 980k |
| -500k | 1.03 | +144k | 7 | 1,053k |
| -1.0M〜-2.0M | 1.51 | +1.41M | 0 | 608k |

**IS/OOS頑健性（grid_floatstop_sweep.py / 2024-04〜2025-04 vs 2025-04〜2026-04, v7構成）**

| ペア | IS PF/net | OOS PF/net | 損失イベント(IS+OOS) |
|------|-----------|------------|----------------------|
| CHFJPY 65/1.0/3/-1.5M | 1.34 / +736k | 2.22 / +676k | 9 |
| NZDJPY 61.8/1.5/7/-1.0M | 2.19 / +大 | 2.65 / +大 | 4 |
| AUDCAD 65/1.0/5/-750k | 5.61 / +大 | 2.92 / +大 | 5 |
...
| 重複防止 | 処理済みイベントID(`_processed_ids`)を `news_processed_ids.json` に永続化（**修正: 旧版はメモリのみ→再起動でイベント窓内に同一指標を再トレードしうるギャップを是正**。14日より古いIDは自動剪定） |
