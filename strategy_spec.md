...
## 6. SMA Squeeze Play (sma_squeeze.py v4.3)

### 概要
SMA200スロープフィルター + SMAスクイーズ解放でトレンドフォロー。v3で日足SMAスロープフィルター追加・COOLDOWN=180分、v4でATR-adaptive trailing stop導入（be_r廃止）、v4.2でtrail_start_multインフラ追加（BT検証により0.0=即時開始が最適と確定）、v4.3でEURJPY停止・T_max=24h追加。
...
|------|------|-----|--------|--------|-----------|-------------|-----|-------|---------------|-----------------|------------|-----------|----------|
| USDJPY | ✅ | 4h | 25 | 150 | 2.0% | 5 | 2.5 | 1.5 | **0.5** | 0.0 | **24h** | 20 | 3 |
