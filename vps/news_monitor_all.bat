@echo off
REM news_monitor_all.bat
REM 経済指標戦略(B+C複合) ライブ監視 v1.1 (magic=20260040)
...
REM
REM パラメータ更新手順:
REM   1. VPS: python optimizer/news_event_bt.py
REM   2. optimizer/news_bt_result.csv 上位行を確認
REM   3. vps/news_monitor.py の PARAMS を更新 -> git push
REM   4. VPS: git pull -> 本 bat を再実行
REM
REM NOTE: pythonw.exe を使用してコンソールクローズ後もデーモン継続
REM       ログ: vps\news_monitor_log_{broker}.txt
REM       Zキャッシュ: vps\news_surprise_cache.json
REM       履歴: data\news_history_{broker}.csv
...
echo news_monitor_all started (axiory, exness).
