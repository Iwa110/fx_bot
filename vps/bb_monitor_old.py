...
改善点（v6からの変更）:
  - BB発注の逐次Discord通知を廃止（ログのみ）
  - Discord通知はsummary_notify.pyの1日4回サマリーに集約
"""
...
HTF_PARAMS = {
    'period':       20,      # 1h足BBの期間
    'sigma':        1.5,     # 1h足BBのσ
    'range_sigma':  1.0,     # これ以内なら「レンジ」と判定（上位足σ位置）
    'bars':         50,      # 取得バー数
}
...
def calc_bb_signal(symbol, is_jpy):
    ...
    if close >= upper:
        ...
        log(symbol + ': BBバンド非到達 σ=' + f'{sigma_pos:+.2f}' + ' ' + htf_reason)
        return None  # バンドタッチなし → シグナルなし
...
def place_order(symbol, sig, logf, webhook):
    ...
    lot      = rm.calc_lot(rm.get_balance(), sl_dist, symbol)
    strategy = 'BB_' + symbol
    ...
    log(msg, logf)
    # Discord通知はサマリー（summary_notify.py）に集約するため逐次通知しない
    return True
