"""
analyze_performance.py  v4
========================================================
【v4の変更点（v3からの修正）】
  OANDA MT5では history_deals_get(from, to) が常に0件返る問題に対応。
  代わりに position_id 指定で個別取得する方式に変更。

  position_idの収集源:
    1. trade_log.json  → orders[].ticket / closed[].ticket (Daily戦略)
    2. bb_log.txt      → 「決済:」行のprofitを直接パース (BB戦略)
    3. 現在のオープンポジション → mt5.positions_get()

  変更した関数:
    - fetch_deals()  → collect_position_ids() + fetch_deals_by_positions() に分割
    - build_trades() → BB戦略はbb_log.txtのprofit行も補完データとして使用

  変更していない関数（v3と同一）:
    - calc_stats / classify_trade / detect_sl_cooldown
    - build_weekly_kpi / print_stats / print_section
    - to_jst / pip_size / get_strategy / get_rr
    - main()の表示ロジック全体
========================================================
"""
...
TRADE_LOG_PATH = os.path.join(BASE, 'trade_log.json')
...
STRATEGY_PREFIXES = [
    ...
    ('STR_',    'STR'),
    ('FXBot_STR', 'STR'),
    ('FXBot_MOM_JPY', 'MOM_JPY'),
    ('FXBot_MOM_GBJ', 'MOM_GBJ'),
]
...
def collect_position_ids() -> dict:
    """
    trade_log.jsonとオープンポジションからposition_idを収集する。
    戻り値: {position_id: {'strategy': str, 'log_profit': float or None}}
      log_profit: trade_log.jsonのclosed[].profitがある場合のみセット
    """
    ids = {}
    ...
    if os.path.exists(TRADE_LOG_PATH):
        try:
            ...
            for closed in data.get('closed', []):
                ...
                if ticket:
                    # profitが0.0の場合はNoneにしてMT5から再取得
                    lp = closed.get('profit')
                    ids[int(ticket)] = {
                        'strategy':   closed.get('strategy', 'UNKNOWN'),
                        'log_profit': lp if (lp is not None and lp != 0.0) else None,
                    }

            print(f'  trade_log.json: {len(ids)}件のticket収集')
        except Exception as e:
            print(f'  trade_log.json読み込みエラー: {e}')

    # ── オープンポジションから収集 ────────────────
    positions = mt5.positions_get()
    if positions:
        for p in positions:
            if p.ticket not in ids:
                ids[int(p.ticket)] = {
                    'strategy':   get_strategy(p.comment),
                    'log_profit': None,
                }
        print(f'  オープンポジション: {len(positions)}件追加')

    return ids


def parse_bb_log_profits() -> list:
    """
    bb_log.txtの「決済:」行からBB戦略の損益を直接パースする。
    OANDAではBBのposition_idがbb_log.txtに記録されないため、
    ログのprofitを補完データとして使用する。

    対象行の例:
      [2026-04-24 10:00:00] 決済: USDJPY PnL=+1760円 BB_USDJPY
      [2026-04-24 10:00:00] 決済: EURUSD PnL=-3200円 BB_EURUSD

    戻り値: [{'symbol', 'profit', 'strategy', 'close_time', 'source'}]
    """
    results = []
    since   = datetime.now() - timedelta(days=ANALYSIS_DAYS)

    # 正規表現: タイムスタンプ・シンボル・PnL・ストラテジーを抽出
    pattern = re.compile(
    ...
    if not os.path.exists(LOG_PATH):
        print(f'  bb_log.txt が見つかりません: {LOG_PATH}')
        return results
    ...
    try:
        with open(LOG_PATH, encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    close_time = datetime.strptime(ts_str, '%Y-%m-%d %H:%M:%S')
                except ValueError:
                    continue
                if close_time < since:
                    continue
        ...
        print(f'  bb_log.txt: {len(results)}件の決済ログ抽出')
    except Exception as e:
        print(f'  bb_log.txt読み込みエラー: {e}')

    return results


# ══════════════════════════════════════════
# v4新規: position指定でdeals取得
# ══════════════════════════════════════════

def fetch_deals_by_positions(position_ids: dict) -> list:
    """
    OANDA MT5はposition指定が効かないため、全期間日時指定で取得し
    position_idでフィルタリングする。
    """
    all_deals = mt5.history_deals_get(datetime(2020, 1, 1), datetime(2030, 1, 1))
    if not all_deals:
        print('  deals取得: 0件')
        return []

    # Depositなど取引以外を除外し、対象position_idに絞る
    filtered = [
        d for d in all_deals
        if d.position_id in position_ids and d.symbol != ''
    ]
    print(f'  deals取得: 全{len(all_deals)}件 → position_idフィルタ後{len(filtered)}件')
    return filtered


# ══════════════════════════════════════════
# v4修正: build_trades（v3のロジックを維持しつつBBログ補完を追加）
# ══════════════════════════════════════════

def build_trades(deals: list, bb_log_profits: list) -> list:
    """
    IN/OUTをposition_idでペアリングしてトレードリストを構築。

    BBについては:
      1. deals内にBBのIN/OUTペアがあればv3と同じロジックで処理
      2. なければbb_log_profitsから補完（profit金額のみの簡易レコード）
    """
    in_deals  = {}
    out_deals = {}
    ...
    trades         = []
    paired_pos_ids = set()  # ペアリング済みのpos_id

    # ── MT5 dealペアリング（Daily戦略・ペアリング成功したBB）────
    for pos_id, out_d in out_deals.items():
        ...
        paired_pos_ids.add(pos_id)

    paired_count = len(trades)
    print(f'  MT5ペアリング: {paired_count}件')

    # ── bb_log.txtから補完（MT5でペアリングできなかったBB決済）────
    # BBのmatch判定: 同じシンボル・同じprofitが既にmt5取引に存在しないか確認
    mt5_bb_profits = set(
        (t['symbol'], t['profit'])
        for t in trades
        if t['strategy'] == 'BB'
    ...
    added_from_log = 0
    for rec in bb_log_profits:
        key = (rec['symbol'], rec['profit'])
        if key in mt5_bb_profits:
            continue  # MT5から既に取得済み → スキップ

        # bb_logのみからでは方向・entry等不明なため最小限フィールドで構築
        profit      = rec['profit']
        tp_expected = 0.0  # bb_logにはTP価格なし → classify_tradeのフォールバック使用
        result      = classify_trade(profit, tp_expected)
        ...
        trades.append({
            ...
            'result':      result,
            'win':         profit > 0,
            'open_time':   rec['close_time'],   # open_timeは不明なのでclose_timeで代用
            'close_time':  rec['close_time'],
            'hold_min':    0,
            'comment':     rec['comment'],
            'source':      'bb_log',
        })
        mt5_bb_profits.add(key)  # 重複防止
        added_from_log += 1

    if added_from_log:
        print(f'  bb_log.txt補完: {added_from_log}件追加')
...
def main():
    print('analyze_performance.py v4 起動')
    ...
    print(f'取得方式: position_id指定（OANDA MT5対応）')
    ...
    # ── v4: position_idベースでデータ収集 ────────────
    print('\n[データ収集]')
    position_ids  = collect_position_ids()
    deals         = fetch_deals_by_positions(position_ids)
    bb_log_profits = parse_bb_log_profits()
    trades        = build_trades(deals, bb_log_profits)

    # ── データソース内訳表示 ──────────────────────────
    mt5_count    = sum(1 for t in trades if t.get('source') == 'mt5')
    bblog_count  = sum(1 for t in trades if t.get('source') == 'bb_log')
    print(f'  取引合計: {len(trades)}件 '
          f'(MT5ペアリング:{mt5_count}件 / bb_log補完:{bblog_count}件)')

    if not trades:
        print('\n分析対象取引なし')
        print('確認事項:')
        print('  1. trade_log.jsonにticketが記録されているか')
        print('  2. bb_log.txtに「決済:」行があるか')
        mt5.shutdown()
        return
    ...
    bblog_only = [t for t in trades if t.get('source') == 'bb_log']
    if bblog_only:
        print(f'\n  ※ bb_log補完分{len(bblog_only)}件は方向不明のためBUY/SELL集計から除外')
