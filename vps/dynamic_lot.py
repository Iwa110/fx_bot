...
def _aggregate_prefixed(stats: dict, prefix: str) -> dict:
    """'BB_*' のような prefix を持つエントリを合算して擬似エントリを返す。"""
    entries = [v for k, v in stats.items() if k.startswith(prefix + '_') or k == prefix]
    if not entries:
        return {}
    ...
    # avg_rr: 勝ちトレードの加重平均
    rr_sum = sum(e.get('avg_rr', 0.0) * e.get('wins', 0) for e in entries)
    total_wins = wins if wins > 0 else 1
    avg_rr = rr_sum / total_wins
    return {'total': total, 'win_rate': win_rate, 'avg_rr': avg_rr}
...
def get_risk_pct(strategy: str) -> float:
    try:
        with open(STATS_PATH, encoding='utf-8') as f:
            stats = json.load(f)
        s = stats.get(strategy) or _aggregate_prefixed(stats, strategy)
        if not s:
            return _FLOOR
        count = s.get('total', 0)
        if count < 20:
