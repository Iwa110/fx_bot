"""
FX 自動発注システム v4
- 戦略：TRI・MOM・STR・CORR（新規追加）
- 最大ポジション：5（各戦略1つ + STR1つ）
- 資金配分：TRI/MOM/CORR 0.1ロット・STR 0.05ロット
- 日4回サマリー・リアルタイム通知
- メールトリガー対応（FX RESET/STOP/STATUS）
"""
...
# ── MOM戦略シグナル ───────────────────────────
def check_mom_signal(params: dict) -> list:
    gbpusd    = get_price('GBPUSD')
    ...
    if not (gbpusd and gbp_rates is not None and eur_rates is not None
            and len(gbp_rates) >= 11):
        return []
...
def check_corr_signal() -> list:
    ...
    z_entry = CORR_PARAMS['z_entry']
    sl_pct  = CORR_PARAMS['sl_pct']
    rr      = CORR_PARAMS['rr']

    # AUD/USD・NZD/USDの終値を取得
    ...
    aud_closes = [r['close'] for r in aud_rates]
    nzd_closes = [r['close'] for r in nzd_rates]

    # AUD/NZD比率のZスコアを計算
    ratios = [a / n for a, n in zip(aud_closes, nzd_closes)]
    ...
    return [{
        ...
        'tp_dist':   tp_dist,
        'sl_dist':   sl_dist,
        'reason':    f"AUD/NZD Zスコア={zscore:.2f}（閾値±{z_entry}）",
    }]
...
def send_summary(log: dict, webhook: str):
    ...
    closed_text = (f"\n本日決済: {len(log['closed'])}回 "
                   f"合計{'+' if total_closed_pnl>=0 else ''}{total_closed_pnl:,.0f}円"
                   if log['closed'] else '')
...
def main():
    ...
    if count_total() >= MAX_TOTAL_POS:
        print(f"合計ポジション上限({MAX_TOTAL_POS})到達。スキップします。")
        mt5.shutdown()
        return
    ...
    all_signals.extend(check_mom_signal(params))
