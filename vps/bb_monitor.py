...
def f1_momentum_filter(df_5m, direction, param):
    ...
    if direction == 'buy':
        # 逆張りBUY: 価格が下がってきている（diff < 0）
        if diff >= 0:
            return False, info_str + ' → 下落モメンタム未確認（BUYスキップ）'
    else:
        # 逆張りSELL: 価格が上がってきている（diff > 0）
        if diff <= 0:
...
def f2_divergence_filter(symbol, direction, div_pips, is_jpy):
    ...
    非JPY系:   EURUSD/GBPUSDとの乖離確認
    div_pips: 最低乖離pips（この値以上の乖離があればフィルター通過）
    戻り値: (ok: bool, reason: str)
    """
...
def f3_bbstack_filter(symbol, direction, sigma_threshold, is_jpy):
    ...
    sigma_threshold: 構成ペアのBBσ位置がこの値以上なら「スタック」と判定
    戻り値: (ok: bool, reason: str)
    """
    def get_bb_sigma(sym):
        bars = mt5.copy_rates_from_pos(sym, mt5.TIMEFRAME_M5, 0, 60)
...
def main():
    ...
    if not mt5.initialize(
        ...
        log('MT5初期化失敗: ' + str(mt5.last_error()), logf)
        return
    ...
    for symbol, cfg in BB_PAIRS.items():
        ...
        # 時間帯フィルター
        allowed = ALLOWED_HOURS_UTC.get(symbol)  # None=制限なし
        if allowed is not None:
            now_hour_utc = datetime.now(timezone.utc).hour
            if now_hour_utc not in allowed:
                log(symbol + ': 時間帯外スキップ UTC=' + str(now_hour_utc) + 'h')
                skipped += 1
                continue
