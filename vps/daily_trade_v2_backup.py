"""
日次戦略スクリプト v2（毎朝7時実行）
- risk_managerを使用してATRベースのロット・TP/SLを動的計算
- 勝率・RRを自動記録（将来のケリー基準準備）
"""
...
def check_daily_loss(log, webhook):
    ...
    if (initial-info.equity)/initial >= 0.05:
        send_discord(f"【FX Bot】{datetime.now().strftime('%Y-%m-%d %H:%M')}\n⛔ 損失上限到達・停止", webhook)
        log['daily_loss_stopped'] = True; save_log(log); return False
    return True

def get_atr(symbol, length=14):
    rates = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_D1, 0, length+5)
    if rates is None or len(rates) < length+1: return None
...
def place_order(symbol, direction, lot, tp_dist, sl_dist, strategy, reason, log, webhook):
    ...
    if result.retcode == mt5.TRADE_RETCODE_DONE:
        ...
        kelly_note = f"\n現在Kelly: {stats.get('kelly',0):.3f}" if stats.get('total',0)>=20 else \
        ...
        send_discord(
            ...
            f"残高: {info.balance:,.0f}円", webhook
        )
        log['orders'].append({
        ...
        save_log(log)
        print(f"発注成功: {strategy} {symbol} {dir_jp} {lot}lot")
        return True
...
def check_mom(symbol, filter_symbol, period=10, mom_th=0.01, filter_th=0.005):
    ...
    if rates is None or frates is None or len(rates)<=period: return None
...
def check_str():
    ...
    for sym,(base,quote) in STR_TICKERS.items():
        ...
        scores[base]=scores.get(base,0)+ret; scores[quote]=scores.get(quote,0)-ret
    if not scores: return None
    sc=sorted(scores.items(),key=lambda x:x[1],reverse=True)
    strongest=sc[0][0]; weakest=sc[-1][0]; spread=sc[0][1]-sc[-1][1]
    ...
    for sym in TRADE_PAIRS:
        ...
        base,quote=STR_TICKERS[sym]; score=0
    ...
    return best_pair, 'buy' if scores.get(base,0)>scores.get(quote,0) else 'sell', \
           f"最強:{strongest} 最弱:{weakest}"

# ── メイン ────────────────────────────────────
def main():
    print(f"[{datetime.now().strftime('%H:%M')}] 日次戦略v2 実行開始")
    config  = load_env()
    webhook = config.get('DISCORD_WEBHOOK','')

    if not mt5.initialize(): print("MT5接続失敗"); return
    info = mt5.account_info()
    if DEMO_MODE and 'demo' not in info.server.lower():
        mt5.shutdown(); return

    log = load_log(); today = str(date.today())
    if log['date']!=today:
        log={'date':today,'initial_balance':info.balance,
             'orders':[],'closed':[],'daily_loss_stopped':False}; save_log(log)
        ...
        if d:
            ...
            if atr:
                ...
                if place_order('USDJPY',d,lot,tp_dist,sl_dist,'MOM_JPY',
                               f"USD/JPYモメンタム ATR={atr:.3f}",log,webhook):
                    executed+=1
        ...
        if d:
            ...
            if atr:
                ...
                if place_order('GBPJPY',d,lot,tp_dist,sl_dist,'MOM_GBJ',
                               f"GBP/JPYモメンタム ATR={atr:.3f}",log,webhook):
                    executed+=1
    ...
    if count_by('CORR')<1 and not is_dup('CORR','AUDUSD',log) and count_total()<MAX_TOTAL:
        ...
        if corr:
            ...
            if atr:
                ...
                lot = rm.calc_lot(balance, sl_dist, 'AUDUSD')
                if place_order('AUDUSD',d,lot,tp_dist,sl_dist,'CORR',
        ...
        if str_sig:
            ...
            if not is_dup('STR', sym, log):
                ...
                if atr:
                    ...
                    if place_order(sym,d,lot,tp_dist,sl_dist,'STR',reason,log,webhook):
                        executed+=1
    ...
    print(f"日次戦略v2完了: 発注{executed}件")
    mt5.shutdown()
